...
import { Upload, User, Home, Trash2, AlertTriangle, Mail, Bell } from "lucide-react";
...
function EmailPreferencesSettings() {
  ...
  const handleFrequencyChange = (value: string) => {
    updateMutation.mutate(value as EmailDigestFrequency);
  };
  ...
  const currentFrequency = preferences?.emailDigestFrequency || "none";
      ...
      <CardContent className="space-y-4">
        <div className="space-y-2">
          ...
          <p className="text-sm text-muted-foreground">
            Receive periodic summaries of new articles across your monitored topics.
          </p>
          <Select
          ...
          >
            <SelectTrigger className="w-48" id="digestFrequency">
              <SelectValue placeholder="Select frequency" />
            </SelectTrigger>
            <SelectContent>
              <SelectItem value="none">No digests</SelectItem>
        ...
        {preferences?.lastDigestSentAt && (
          <p className="text-xs text-muted-foreground">
            Last digest sent: {new Date(preferences.lastDigestSentAt).toLocaleDateString()}
          </p>
        )}
      </CardContent>
    </Card>
...
function NotificationSettings() {
  ...
  const handleAlertsToggle = () => {
    updateMutation.mutate({ alertsEnabled: !settings?.alertsEnabled });
  };

  const handleMethodChange = (value: string) => {
    updateMutation.mutate({ alertMethod: value as "email" | "in_app" | "both" });
  };
      ...
      <CardContent className="space-y-6">
        <div className="flex items-center justify-between">
          <div className="space-y-0.5">
            ...
            <p className="text-sm text-muted-foreground">
              Receive notifications when new articles match your monitored topics.
            </p>
          </div>
          <Button
            id="alertsEnabled"
        ...
        {settings?.alertsEnabled && (
          <div className="space-y-2">
            <Label htmlFor="alertMethod">Alert Delivery Method</Label>
            <p className="text-sm text-muted-foreground">
              Choose how you want to receive new article alerts.
            </p>
            <Select
              value={settings?.alertMethod || "email"}
              onValueChange={handleMethodChange}
              disabled={updateMutation.isPending}
            >
              <SelectTrigger className="w-48" id="alertMethod">
        ...
        {settings?.lastAlertSentAt && (
          <p className="text-xs text-muted-foreground">
            Last alert sent: {new Date(settings.lastAlertSentAt).toLocaleDateString()}
          </p>
        )}
      </CardContent>
    </Card>
